...
class Game:
	def __init__(self):
		self.frame = 0
		self.ball = Ball()
		self.arcs = [Arc(i * SPACING, 0) for i in range(5, 40)]
		self.particles = []
	...
	def run(self):
		...
		for arc in self.arcs:
			arc.angle += (arc.radius + 1500) / 50000
			arc.update()
		...
		for i, arc in enumerate(self.arcs):
			...
			if collResult == 1:
				...
				if self.ball.last != arc:
					self.ball.last = arc
					boost = (random.random() + 1)
					self.ball.vx *= boost
					self.ball.vy *= boost
					norm = self.ball.vx * self.ball.vx + self.ball.vy * self.ball.vy
					if norm > HIT_SPEED * HIT_SPEED:
						norm = HIT_SPEED / math.sqrt(norm)
						self.ball.vx *= norm
						self.ball.vy *= norm
