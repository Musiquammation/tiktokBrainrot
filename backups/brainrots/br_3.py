...
class Arc:
	...
	def __init__(self, radius: float, angle: float, angle_length: float):
		self.radius = radius
		self.angle = angle
		self.target_radius = radius
		self.angle_length = angle_length
		self.color = gradient(Arc.index / 20)
...
class Game:
	def __init__(self):
		...
		for i in range(10, 11):
			a += .3
			self.arcs.append(Arc(i * SPACING, a, math.pi * 1.92))
	...
	def run(self):
		...
		for arc in self.arcs:
			arc.angle += math.fabs(math.cos(arc.angle) + math.sin(arc.angle))*.01 + .003
			# arc.radius -= .1
			arc.update()
	...
	def check_collisions(self):
		...
		for i, arc in enumerate(self.arcs):
			collResult = collision_ball_arc(self.ball.x, self.ball.y, self.ball.size, arc.radius, arc.angle, arc.angle + arc.angle_length)
			...
			if collResult == 1:
				...
				correct_distance = arc.radius - self.ball.size
			...
			if collResult == -1:
				...
				for j in range(num_particles):
					...
					t = j / (num_particles - 1)
					angle_on_arc = arc.angle + t * arc.angle_length
	...
	def draw(self, screen: pygame.Surface):
		...
		pygame.draw.rect(screen, (15, 0, 0), (0, dumb_y, WIDTH, HEIGHT - dumb_y))

		# Draw arcs
		for i, arc in enumerate(self.arcs):
			radius = arc.radius
			start_angle = arc.angle
			end_angle = start_angle + arc.angle_length
		...
		# ====== TITRE PRINCIPAL ======
		title_text = "WHERE WILL THE BALL ESCAPE?"
